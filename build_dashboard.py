...
function openBrief(id,biz){
  ...
  const aiWeaknesses=aiConItems.length?[aiConItems.map(function(c){return '- '+c;}).join('\n')+(aiOpp?'\n\n'+aiOpp:'')]:[];
  // ── OUTREACH data ──────────────────────────────────────────────────────────
  // Convert "Best call times" from ET to Da Nang (VNT = UTC+7, EDT = UTC-4, offset +11h)
  function toVNT(raw){
    return raw.replace(/(best call times?[:\s]+)([^\n]+)/gi,function(match,prefix,timesPart){
      var conv=timesPart.replace(/(\d{1,2})\s*[–\-]\s*(\d{1,2})\s*(am|pm)/gi,function(orig,h1,h2,ap){
        function fmt(h,ampm){
          h=parseInt(h);
          if(ampm.toLowerCase()==='pm'&&h!==12)h+=12;
          if(ampm.toLowerCase()==='am'&&h===12)h=0;
          var vnt=(h+11)%24;var over=(h+11)>=24;
          var suf=vnt>=12?'PM':'AM';var dh=vnt>12?vnt-12:(vnt===0?12:vnt);
          return dh+suf+(over?' (+1d)':'');
        }
        return fmt(h1,ap)+'–'+fmt(h2,ap)+' VNT ['+orig+' ET]';
      });
      return prefix+conv;
    });
  }
  const notesHtml=m(toVNT(sec(p.brief,'NOTES')));
  ...
  // Why a Site? — custom sales copy (bold=yellow standout, italic=green takeaway)
  const B='<strong style="color:#f5c400;font-weight:700">';
  const I='<em style="color:#00ff9d;font-style:normal;font-weight:600">';const IE='<\/em>';
  const PT='<div style="margin-bottom:18px;padding:14px 16px;background:#0a1322;border-radius:4px;border-left:3px solid rgba(0,212,255,.2);font-size:13px;line-height:1.75;color:#c8d8e8">';
  const SHG='<div style="font-family:\'Syne\',sans-serif;font-size:11px;font-weight:700;letter-spacing:2px;text-transform:uppercase;padding:10px 14px;margin:0 -2px 14px;border-radius:3px;background:rgba(0,255,157,.07);border-left:3px solid #00ff9d;color:#00ff9d">';
  const SHR='<div style="font-family:\'Syne\',sans-serif;font-size:11px;font-weight:700;letter-spacing:2px;text-transform:uppercase;padding:10px 14px;margin:8px -2px 14px;border-radius:3px;background:rgba(248,113,113,.07);border-left:3px solid #f87171;color:#f87171">';
  const anglesHtml=''
    +SHG+'&#x1F504; REDESIGNED — NEW WEBSITE<\/div>'
    +PT+B+'The website is your silent salesperson.<\/strong> 97% of consumers search online before hiring a local service business — and they\'ve already made the decision before picking up the phone. Especially in high-income zip codes, affluent homeowners vet trade businesses like they vet financial advisors.<br><br>'+I+'A cheap or sloppy website loses the job before you even get to quote it.'+IE+'<\/div>'
    +PT+B+'Local SEO is a winner-take-most game.<\/strong> The top 3 local results get 75% of clicks. A properly optimised website with service pages — "Roof Replacement in [High-Income City]" — blocks out competitors for years.<br><br>'+I+'Getting to page 1 is compounding — it gets easier to stay there.'+IE+'<\/div>'
    +PT+B+'Premium clients pay more when the brand looks premium.<\/strong> A $150/hr electrician needs a $150/hr-looking website. Pricing perception starts at the website.<br><br>'+I+'Cheap-looking = lowball offers and price shoppers. Professional-looking = customers who won\'t negotiate.'+IE+'<\/div>'
    +SHR+'&#x274C; CURRENTLY NO WEBSITE<\/div>'
    +PT+B+'Recurring &amp; referral business needs a home base.<\/strong> Turns word-of-mouth into booked jobs automatically. Your referral network becomes an automated booking channel and sales funnel.<\/div>'
    +PT+B+'Your Google Business Profile alone isn\'t enough. It\'s a listing. Your website is a brand.<\/strong> Without a quality website you can\'t rank for high-intent keywords like "emergency HVAC repair in [city]." GBP can be suppressed — your website can\'t. You can\'t control the narrative.<\/div>'
    +PT+B+'Collect leads 24/7.<\/strong> A website with a contact form or booking widget captures after-hours leads while competitors miss them entirely.<br><br>'+I+'An emergency at 11pm sends someone to Google — not to voicemail.'+IE+'<\/div>'
    +PT+B+'Competitors are already doing it — and winning.<\/strong> Trades businesses with strong branded websites absorb market share. Every month without a website is compounding lost revenue.<br><br>'+I+'The best time to build one was two years ago. The second best time is today.'+IE+'<\/div>'
    +PT+B+'Social proof lives on your website and builds trust.<\/strong> Reviews, certifications, photos of completed jobs, and before/after galleries turn browsers into booked calls.<\/div>'
    +PT+B+'Your website is the only platform you own.<\/strong> Google owns your GBP. Yelp owns your listing. They can be suppressed or shut down at any time. Your website belongs to you — permanently.<\/div>';
  // ── INTEL tab ──────────────────────────────────────────────────────────────
  ...
  const SOL={
    searchVis:'- Service pages with local keywords put you in front of people searching right now\n- Your 5-star reviews show up where buyers are looking — not just word of mouth\n- Competitors ranking above you have this. You don\'t — yet.',
    bookingFriction:'- Quote form + click-to-call on every page\n- A ready-to-hire visitor can act immediately\n- Right now they bounce to whoever makes it easiest',
    serviceClarity:'- A dedicated page per service captures that specific search\n- Makes it instantly clear what you do and where you work\n- One page = one keyword = one ranking',
    emergencyGap:'- Prominent emergency line + 24/7 badge on every page\n- Captures urgent $500–$1,500 jobs before a competitor answers\n- You stop losing after-hours calls',
    competitorAdv:'- Fast, mobile-first site with trust signals and real reviews\n- Outranks directory listings and weak local competitors\n- Converts their search traffic into your leads',
    auditVerdict:'- Faster load, mobile-first layout, proper schema — day one\n- Clear contact structure search engines can actually read\n- Fixes every missing signal the audit flagged',
    aiVisibility:'- Schema + FAQ content gets you into Google AI Overviews\n- Neighborhood pages signal local expertise to AI tools\n- The next generation searches AI before they call — be there'
  };
  ...
  // Convert a prose pain block into ≤3 punchy bullet points
  function bulletify(text){
    if(!text) return text;
    // Already bulleted (AI visibility items) — leave alone
    if(/^\s*[-*]/.test(text)||text.indexOf('\n-')>-1) return text;
    // Split on sentence boundaries: ". Capital" or " — Capital"
    var parts=text.replace(/([.!?])\s+([A-Z])/g,'$1\n$2').replace(/\s+—\s+([A-Z])/g,'\n$1').split('\n');
    parts=parts.map(function(s){return s.trim();}).filter(function(s){return s.length>12;});
    if(parts.length===0) return '- '+text.trim();
    // Take first 3 meaningful sentences
    return parts.slice(0,3).map(function(s){return '- '+s;}).join('\n');
  }
  function buildWarRoom(painSolutions,hooks,reviewItems){
    ...
    if(painSolutions.length){
      ...
      painSolutions.forEach(function(item){
        ...
        html+='</div>';
        html+='<div style="font-size:13px;color:'+textC+';line-height:1.7" class="md-pain">'+m(bulletify(item.text))+'</div>';
        html+='</div>';
        // RIGHT — Our Fix
        ...
        html+='</div>';
        html+='<div style="font-size:13px;color:#8ab8a8;line-height:1.7" class="md-pain">'+m('- '+item.solution)+'</div>';
        html+='</div>';
        html+='</div>';
      });
    }
    ...
    if(hooks.length){
      ...
      hooks.forEach(function(h,i){
        ...
        html+='<button id="'+btnId+'" onclick="doCopy(\''+btnId+'\',\''+txtId+'\')" style="background:rgba(0,255,157,.08);border:1px solid rgba(0,255,157,.25);color:#00ff9d;padding:4px 12px;border-radius:2px;font-family:\'DM Sans\',sans-serif;font-size:11px;font-weight:600;cursor:pointer;white-space:nowrap;flex-shrink:0">&#x1F4CB; Copy</button>';
        html+='</div>';
  ...
  function buildCompScorecard(compRaw,aiRaw,auditRaw){
    ...
    function det(text){
      if(!text) return '';
      var t=text.replace(/\n/g,' ').trim();
      // Split on first natural boundary — take the first clause, no ellipsis
      var bounds=[' — ',' = ','; '];
      for(var bi=0;bi<bounds.length;bi++){
        var idx=t.indexOf(bounds[bi]);
        if(idx>20){t=t.substring(0,idx).trim();break;}
      }
      // If still very long, break at last word boundary before 110 chars (no ellipsis)
      if(t.length>110){
        var cut=t.substring(0,110);
        var sp=cut.lastIndexOf(' ');
        if(sp>70)t=cut.substring(0,sp);
      }
    ...
    var rows=[
      ...
      {icon:'⚔',label:'Your Advantage',
       ...
       cTag:function(c){return 'miss';},cLabel:function(c){return 'Weakness';},cDet:function(c){return c.weak;}},
  ...
  const portalHtml='<!DOCTYPE html><html><head><meta charset="UTF-8"/><title>'+esc(biz)+' &#x2014; Sales Portal</title>'
    ...
    +'<button class="tab-btn" onclick="window.open(\'./QF_Sales_Call_Script.html\',\'_blank\')">&#x1F4CB; Call Script</button>'
    +'</div>'
    // ── OVERVIEW TAB
    ...
    // ── STRATEGY TAB (unused — Call Script opens in new browser tab)
    +'<div id="tab-strategy" class="tab-body"></div>'
    // ── Modal
    ...
    +'<div class="modal-overlay" id="glossaryModal"><div class="modal-box" style="max-width:760px">'
    +'<button class="modal-close" onclick="closeGlossaryModal()">&#x2715;</button>'
    +'<div class="modal-title">&#x1F4D6; Website &amp; SEO Glossary</div>'
    +'<div class="glossary-term"><h3>&#x1F4CC; Index / Indexed Content</h3><ul>'
    +'<li>When a search engine (Google, Bing) has <strong>crawled, read, and stored</strong> your page in its database — only indexed pages can appear in search results.</li>'
    +'<li>If a page isn\'t indexed, it is <strong>invisible to every searcher</strong>, regardless of how good the content is.</li>'
    +'</ul><p class="eg">Example: A brand-new site with no backlinks and no sitemap may sit unindexed for weeks. Our builds include a sitemap.xml and proper crawl signals so Google finds the site immediately.</p></div>'
    +'<div class="glossary-term"><h3>&#x1F4CC; JSON-LD Schema</h3><ul>'
    +'<li>A block of <strong>invisible code</strong> (in the page &lt;head&gt;) that tells search engines exactly what your business is: name, address, phone, hours, service area, trade type.</li>'
    +'<li>Without it, Google <strong>guesses</strong>. With it, you control what it says — and it powers rich results (star ratings, address in search snippets).</li>'
    +'</ul><p class="eg">Example: "LocalBusiness" + "Plumber" schema ensures Google knows you\'re a licensed plumber in Financial District, not a plumbing supply store in New Jersey.</p></div>'
    +'<div class="glossary-term"><h3>&#x1F4CC; AI Visibility</h3><ul>'
    +'<li>Whether AI tools like <strong>ChatGPT, Gemini, and Google AI Overviews</strong> recommend your business when someone asks "best plumber near me."</li>'
    +'<li><strong>What determines it:</strong></li>'
    +'<li>① Editorial citations — roundup articles, local press, directory listings that mention your business by name</li>'
    +'<li>② Structured data / JSON-LD Schema — tells AI what you do and where</li>'
    +'<li>③ FAQ content — pages that directly answer questions AI tools are trained to surface</li>'
    +'<li>④ E-E-A-T signals — named owner/operator, credentials, years in business, real reviews</li>'
    +'<li>⑤ Neighborhood-specific pages — AI cites hyper-local content for local queries</li>'
    +'</ul><p class="eg">Example: A rep asks ChatGPT "best electrician in Manhattan" — only businesses with schema, FAQ pages, and editorial mentions get cited. Most trade contractors have none of these.</p></div>'
    +'<div class="glossary-term"><h3>&#x1F4CC; Meta Tags</h3><ul>'
    +'<li>HTML labels on each page that tell search engines the <strong>title</strong> and <strong>description</strong> — the two lines that show under your link in Google results.</li>'
    +'<li>A missing or generic meta description means Google writes one itself — usually a random sentence pulled from the page.</li>'
    +'</ul><p class="eg">Example: "Manhattan Plumber — 5★ Rated | A&E NYC Plumbing | Same-Day Emergency Service" vs. a blank title that defaults to the domain name.</p></div>'
    +'<div class="glossary-term"><h3>&#x1F4CC; SEO — Search Engine Optimization</h3>'
    +'<p class="glossary-seo-cat">① Technical SEO</p><ul>'
    +'<li>Site speed, mobile-readiness, crawlability, HTTPS, Core Web Vitals, sitemap, robots.txt</li>'
    +'<li>The foundation — if this is broken, nothing else matters</li>'
    +'</ul><p class="glossary-seo-cat">② On-Page SEO</p><ul>'
    +'<li>Keywords in headings, page titles, URLs, image alt text, and body copy</li>'
    +'<li>Each service needs its own page targeting its own keyword (e.g. "boiler repair NYC")</li>'
    +'</ul><p class="glossary-seo-cat">③ Local SEO</p><ul>'
    +'<li>Google Business Profile optimization, NAP consistency (Name/Address/Phone identical everywhere), service area pages, local citations</li>'
    +'<li>The most impactful category for trade businesses — this is what drives map pack rankings</li>'
    +'</ul><p class="glossary-seo-cat">④ Authority / Off-Page SEO</p><ul>'
    +'<li>Backlinks from other sites, editorial mentions, directory listings, local press</li>'
    +'<li>Tells Google this business is trusted by others — harder to fake, highest long-term value</li>'
    +'</ul></div>'
    +'<div class="glossary-term"><h3>&#x1F4CC; Optimization / Mobile Optimization</h3><ul>'
    +'<li><strong>Optimization</strong> = reducing file sizes, eliminating render-blocking code, and streamlining page load so the site is fast and efficient.</li>'
    +'<li><strong>Mobile optimization</strong> = the site layout, buttons, and text automatically adapt to phone screens — tap targets are large enough, text is readable without zooming, no horizontal scrolling.</li>'
    +'<li>70%+ of local service searches happen on mobile. Google uses <strong>mobile-first indexing</strong> — your phone version is what gets ranked.</li>'
    +'</ul><p class="eg">Example: A site that loads in 1.2s on mobile ranks above an identical site that loads in 4.8s.</p></div>'
    +'<div class="glossary-term"><h3>&#x1F4CC; Site Audit</h3><ul>'
    +'<li>A systematic scan of a website\'s <strong>technical health</strong> — identifying everything that hurts its ability to rank and convert visitors.</li>'
    +'<li><strong>What\'s checked:</strong> broken links, missing schema, page speed scores, mobile compatibility, missing/duplicate meta tags, indexability issues, missing SSL, thin content pages.</li>'
    +'<li><strong>How they\'re done:</strong> Tools like Google Search Console (free), Ahrefs Site Audit, or Screaming Frog crawl every page and flag issues with severity scores.</li>'
    +'</ul><p class="eg">Example: Our site audit on a prospect\'s current site found no JSON-LD, no FAQ page, no booking form, and a 6.2s mobile load time — all fixed on day one with the QuickFlip build.</p></div>'
    +'<div class="glossary-term"><h3>&#x1F4CC; Structured Page Architecture</h3><ul>'
    +'<li>Organizing a website so each <strong>service has its own dedicated page</strong>, with a clear hierarchy: Home → Services → [Each Service] → [Each Service Area].</li>'
    +'<li>Search engines rank <strong>individual pages</strong>, not whole sites. "Boiler repair in Financial District" needs its own page to rank for that search — a single homepage can\'t rank for 20 different services in 10 different neighborhoods.</li>'
    +'<li>Structured architecture also signals to AI tools that you\'re a <strong>specialist</strong> in each area, not a generalist.</li>'
    +'</ul><p class="eg">Example: Hub Plumbing has a Boiler Division — that should be a dedicated section with its own pages for boiler repair, boiler replacement, boiler inspections, and violation removal. Currently it\'s buried in a dropdown.</p></div>'
    +'</div></div></div>'
    +'<div class="float-bar">'
    +'<button class="float-btn3" onclick="openPostSale(this)">&#x1F4B0; Post Sale!</button>'
    +'<button class="float-btn2" onclick="openGlossaryModal()">&#x1F4D6; Web Glossary</button>'
    +'<button class="float-btn" onclick="openWhyModal()">&#x1F4A1; Why a Site?</button>'
    +'</div>'
    +'<script>function showTab(btn,id){document.querySelectorAll(".tab-body").forEach(function(t){t.style.display="";t.classList.remove("active")});document.querySelectorAll(".tab-btn").forEach(function(b){b.classList.remove("active")});document.getElementById("tab-"+id).classList.add("active");btn.classList.add("active");}function showOTab(btn,id){document.querySelectorAll(".otab-pane").forEach(function(p){p.classList.remove("active")});document.querySelectorAll(".otab-btn").forEach(function(b){b.classList.remove("active")});var el=document.getElementById("otab-"+id);if(el)el.classList.add("active");btn.classList.add("active");}function scriptNav(btn,id){var f=document.getElementById("stratIframe");if(f&&f.contentDocument){var el=f.contentDocument.getElementById(id);if(el)el.scrollIntoView({behavior:"smooth",block:"start"});}document.querySelectorAll(".snav-btn").forEach(function(b){b.classList.remove("active")});btn.classList.add("active");}function doCopy(btnId,txtId){var t=document.getElementById(txtId).innerText;if(navigator.clipboard){navigator.clipboard.writeText(t).then(function(){var b=document.getElementById(btnId);b.textContent="Copied!";setTimeout(function(){b.textContent="📋 Copy";},2000);});}else{var r=document.createRange();r.selectNodeContents(document.getElementById(txtId));window.getSelection().removeAllRanges();window.getSelection().addRange(r);document.execCommand("copy");}}function openWhyModal(){document.getElementById("whyModal").classList.add("open");}function closeWhyModal(){document.getElementById("whyModal").classList.remove("open");}document.getElementById("whyModal").addEventListener("click",function(e){if(e.target===this)closeWhyModal();});function openGlossaryModal(){document.getElementById("glossaryModal").classList.add("open");}function closeGlossaryModal(){document.getElementById("glossaryModal").classList.remove("open");}document.getElementById("glossaryModal").addEventListener("click",function(e){if(e.target===this)closeGlossaryModal();});function openPostSale(btn){var emojis=["💰","💵","🤑","💸","✅","🎉"];var r=btn.getBoundingClientRect();for(var i=0;i<10;i++){(function(i){setTimeout(function(){var p=document.createElement("div");p.className="money-particle";p.textContent=emojis[Math.floor(Math.random()*emojis.length)];p.style.left=(r.left+Math.random()*r.width)+"px";p.style.top=(r.top-10)+"px";p.style.animationDuration=(0.8+Math.random()*0.7)+"s";document.body.appendChild(p);setTimeout(function(){p.remove();},1500);},i*80);})(i);}setTimeout(function(){var w=window.open("","_blank");var biz=document.querySelector(".hdr-title")?document.querySelector(".hdr-title").innerText:"Client";var html=window.opener?window.opener.buildPostSaleHtml(biz):buildPostSaleHtml(biz);w.document.open();w.document.write(html);w.document.close();},300);}buildPostSaleHtml_placeholder(){}<\/script>'
    +'</body></html>';
...
function buildPostSaleHtml(biz){
  var OC='<span style="font-size:9px;font-weight:600;padding:3px 7px;border-radius:10px;background:rgba(245,196,0,.15);color:#f5c400;border:1px solid rgba(245,196,0,.3)">Client</span>';
  var OZ='<span style="font-size:9px;font-weight:600;padding:3px 7px;border-radius:10px;background:rgba(192,132,252,.15);color:#c084fc;border:1px solid rgba(192,132,252,.3)">Zee</span>';
  var OD='<span style="font-size:9px;font-weight:600;padding:3px 7px;border-radius:10px;background:rgba(0,212,255,.12);color:#00d4ff;border:1px solid rgba(0,212,255,.25)">Dev</span>';
  var CARD='background:#111c35;border:1px solid rgba(0,212,255,.2);border-left:5px solid #10b981;border-radius:8px;padding:20px 24px;margin-bottom:14px';
  var HDR='display:flex;align-items:center;gap:12px;margin-bottom:14px;flex-wrap:wrap';
  var NUM='width:34px;height:34px;min-width:34px;border-radius:50%;background:linear-gradient(135deg,#10b981,#00c896);color:#060b18;font-family:Syne,sans-serif;font-size:14px;font-weight:800;display:flex;align-items:center;justify-content:center;box-shadow:0 0 12px rgba(16,185,129,.4)';
  var TTL='font-family:Syne,sans-serif;font-size:14px;font-weight:700;color:#00ff9d;flex:1';
  var OWNS='display:flex;gap:4px;flex-wrap:wrap';
  var MAIN='font-size:15px;color:#d0e0f0;line-height:1.8;margin-bottom:10px';
  var TECH='font-size:11px;color:#3a5a70;line-height:1.7;border-top:1px solid rgba(0,212,255,.08);padding-top:8px';
  function step(n,title,owns,main,tech){
    return '<div style="'+CARD+'"><div style="'+HDR+'"><div style="'+NUM+'">'+n+'</div><div style="'+TTL+'">'+title+'</div><div style="'+OWNS+'">'+owns+'</div></div><div style="'+MAIN+'">'+main+'</div><div style="'+TECH+'">'+tech+'</div></div>';
  }
  var H2='font-family:Syne,sans-serif;font-size:17px;font-weight:700;color:#00d4ff;letter-spacing:1px;margin:32px 0 14px;padding-bottom:8px;border-bottom:1px solid rgba(0,212,255,.15)';
  var tls='<span style="font-size:9px;font-weight:600;padding:2px 6px;border-radius:10px;';
  var tlC=tls+'background:rgba(245,196,0,.12);color:#f5c400;border:1px solid rgba(245,196,0,.25)">Client</span>';
  var tlZ=tls+'background:rgba(192,132,252,.12);color:#c084fc;border:1px solid rgba(192,132,252,.25)">Zee</span>';
  var tlD=tls+'background:rgba(0,212,255,.1);color:#00d4ff;border:1px solid rgba(0,212,255,.2)">Dev</span>';
  var css='*{box-sizing:border-box;margin:0;padding:0}body{font-family:"DM Sans",sans-serif;background:#060b18;color:#c8d8e8;min-height:100vh;padding:0 0 60px}'
    +'.tl-wrap{position:relative;display:flex;align-items:flex-start;background:#0d1528;border:1px solid rgba(0,212,255,.12);border-radius:6px;padding:24px 16px 20px;margin-bottom:28px}'
    +'.tl-line{position:absolute;top:43px;left:9%;right:9%;height:2px;background:linear-gradient(90deg,#10b981,rgba(0,212,255,.4))}'
    +'.tl-step{flex:1;display:flex;flex-direction:column;align-items:center;position:relative;z-index:1;padding:0 4px}'
    +'.tl-num{width:36px;height:36px;border-radius:50%;background:linear-gradient(135deg,#10b981,#00c896);color:#060b18;font-family:Syne,sans-serif;font-size:14px;font-weight:800;display:flex;align-items:center;justify-content:center;margin-bottom:8px;box-shadow:0 0 10px rgba(16,185,129,.35)}'
    +'.tl-label{font-size:10px;font-weight:600;color:#7aa0b8;text-align:center;margin-bottom:6px;line-height:1.4}'
    +'.tl-owners{display:flex;flex-wrap:wrap;gap:3px;justify-content:center}'
    +'.pcard{background:#0d1528;border:1px solid rgba(0,212,255,.15);border-top:4px solid #10b981;border-radius:8px;padding:22px}'
    +'.pcard h3{font-family:Syne,sans-serif;font-size:15px;font-weight:700;color:#c8d8e8;margin-bottom:2px}'
    +'.pcard .sub{font-size:11px;color:#4a7080;margin-bottom:10px}'
    +'.pcard .price{font-size:26px;font-weight:700;color:#10b981;margin:10px 0 6px}'
    +'.pcard .pitch{font-size:13px;color:#7ab0c0;line-height:1.6;font-style:italic;margin-bottom:12px}'
    +'.pcard ul{list-style:disc;margin-left:18px}'
    +'.pcard li{font-size:12px;color:#8ab0c8;line-height:1.6;margin-bottom:3px}'
    +'.alert{background:rgba(245,158,11,.07);border-left:4px solid #f59e0b;border-radius:0 6px 6px 0;padding:12px 18px;margin:16px 0;font-size:13px;color:#c8a840;line-height:1.6}'
    +'.upsell{margin:0 0 0 18px}.upsell li{font-size:13px;color:#8ab0c8;line-height:1.6;margin-bottom:8px}.upsell strong{color:#f5c400}';
  var h='<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Post Sale &#x2014; '+biz+'</title>'
    +'<link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@300;400;500;600&family=Syne:wght@700;800&display=swap" rel="stylesheet">'
    +'<style>'+css+'</style></head>'
    +'<body style=\'font-family:"DM Sans",sans-serif;background:#060b18;color:#c8d8e8\'>';
  h+='<div style="background:linear-gradient(135deg,#0a1f0a,#0d1f2d);border-bottom:2px solid rgba(16,185,129,.4);padding:28px 40px">'
    +'<div style="font-family:Syne,sans-serif;font-size:26px;font-weight:800;color:#10b981;letter-spacing:1px;margin-bottom:4px">&#x1F4B0; Post Sale &mdash; Next Steps</div>'
    +'<div style="font-size:14px;color:#4a9070;font-weight:500">Client: '+biz+' &nbsp;&middot;&nbsp; Walk through this with your client after they buy</div>'
    +'</div>';
  h+='<div style="max-width:900px;margin:0 auto;padding:32px 40px">';
  h+='<h2 style="'+H2+'">How the Process Works</h2>';
  h+='<div class="tl-wrap"><div class="tl-line"></div>'
    +'<div class="tl-step"><div class="tl-num">1</div><div class="tl-label">Onboarding &amp; Info Gathering</div><div class="tl-owners">'+tlC+tlZ+tlD+'</div></div>'
    +'<div class="tl-step"><div class="tl-num">2</div><div class="tl-label">Development &amp; Build</div><div class="tl-owners">'+tlD+'</div></div>'
    +'<div class="tl-step"><div class="tl-num">3</div><div class="tl-label">QA &amp; Client Review</div><div class="tl-owners">'+tlD+'</div></div>'
    +'<div class="tl-step"><div class="tl-num">4</div><div class="tl-label">Deployment &amp; Handover</div><div class="tl-owners">'+tlD+'</div></div>'
    +'<div class="tl-step"><div class="tl-num">5</div><div class="tl-label">Post-Launch Support</div><div class="tl-owners">'+tlZ+tlD+tlC+'</div></div>'
    +'</div>';
  h+='<div style="text-align:center;margin-bottom:32px">'
    +'<a href="https://forms.gle/RYJQ1JQ3eMJQXbA97" target="_blank" style="display:inline-block;background:linear-gradient(135deg,rgba(16,185,129,.2),rgba(0,212,255,.1));border:2px solid rgba(16,185,129,.5);color:#10b981;padding:14px 36px;border-radius:6px;font-family:Syne,sans-serif;font-size:15px;font-weight:700;letter-spacing:.5px;text-decoration:none">&#x1F4CB;&nbsp; Onboarding Form for Client &nbsp;&#x2192;</a>'
    +'<div style="font-size:12px;color:#2a5a48;margin-top:8px">Send this link to your client right now &mdash; takes 5&ndash;10 minutes to fill out</div>'
    +'</div>';
  h+='<h2 style="'+H2+'">Step-by-Step Breakdown</h2>';
  h+=step(1,'Onboarding &amp; Information Gathering',OC+OZ+OD,
    "We'll send you a quick form to fill out. It covers your logo, your business details, the services you want listed, and any photos you'd like on the site. Once that's done, we have everything we need to get started. Takes about 5&ndash;10 minutes on your end.",
    '<strong>What we collect:</strong> Domain access &middot; Logo &amp; brand colors &middot; Service list &middot; Address &amp; phone &middot; Any photos or existing copy');
  h+=step(2,'Development, Configuration &amp; Optimization',OD,
    "Our team takes everything you sent and builds the site. You don't need to do anything here &mdash; we handle the design, your service pages, your contact form, and make sure Google can find you. We'll have a preview ready for you shortly.",
    '<strong>Under the hood:</strong> Hosting &amp; environment setup &middot; Core pages (Home, Services, Contact) &middot; Lead intake form &middot; Image optimization &middot; Meta tags &amp; sitemap &middot; SSL certificate');
  h+=step(3,'Quality Assurance &amp; Client Review',OD,
    "Before anything goes live, we send you a preview link so you can see exactly how the site looks. Take a look, tell us what you'd like changed &mdash; a photo, some wording, a color. We fix it. Nothing goes live until you're happy with it.",
    '<strong>What we check:</strong> Mobile, tablet &amp; desktop testing &middot; Form submissions &middot; Page load speed &middot; Staging link sent for your approval');
  h+=step(4,'Deployment &amp; Handover',OD,
    "Once you've approved it, we flip the switch. Your site goes live at your web address. We'll walk you through what you've got &mdash; where to find things and how to reach us if something comes up.",
    '<strong>Going live:</strong> DNS pointed to the new site &middot; Production mode enabled &middot; Login credentials delivered &middot; Quick orientation walkthrough');
  h+=step(5,'Post-Launch Support',OZ+OD+OC,
    "We keep an eye on things for the first few weeks after launch. If anything breaks or you want a small change, just reach out and we've got you. Your site is backed up automatically so nothing is ever lost.",
    '<strong>Behind the scenes:</strong> Uptime monitoring &middot; Automated backups &middot; Bug patching &middot; 1&ndash;3 month support window');
  h+='<h2 style="'+H2+'">Website Care &amp; Maintenance Plans &mdash; Pitch These</h2>';
  h+='<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:16px;margin-top:16px">';
  h+='<div class="pcard">'
    +'<h3>Basic Care</h3><div class="sub">Simple / Static Sites</div>'
    +'<div class="price">$99 <span style="font-size:14px;font-weight:400;color:#5a8090">/mo</span></div>'
    +'<p class="pitch">"Your site stays up, stays protected, and you never think about it again. We handle everything in the background."</p>'
    +'<ul><li>Guaranteed uptime &mdash; always online</li><li>Content updates (&lt;30 min/mo)</li><li>Domain &amp; SSL auto-renewed</li><li>Best for simple, stable sites</li></ul>'
    +'</div>';
  h+='<div class="pcard" style="border-top-color:#3b82f6">'
    +'<h3>Standard Care</h3><div class="sub">Dynamic / SMB Sites</div>'
    +'<div class="price" style="color:#60a5fa">$175 <span style="font-size:14px;font-weight:400;color:#5a8090">/mo</span></div>'
    +'<p class="pitch">"We watch your traffic, keep your Google ranking healthy, and back everything up automatically. Most growing businesses end up here."</p>'
    +'<ul><li>Analytics monitoring &mdash; see who\'s finding you</li><li>Automated backups so nothing is lost</li><li>Bug fixes &amp; small tweaks included</li><li>Monthly SEO &amp; speed tuning</li></ul>'
    +'</div>';
  h+='<div class="pcard" style="border-top-color:#8b5cf6">'
    +'<h3>Premium Care</h3><div class="sub">E-commerce / Complex</div>'
    +'<div class="price" style="color:#a78bfa">$250 <span style="font-size:14px;font-weight:400;color:#5a8090">/mo</span></div>'
    +'<p class="pitch">"24/7 monitoring. If something breaks, it gets fixed immediately &mdash; not tomorrow. For high-volume businesses where every hour offline costs money."</p>'
    +'<ul><li>Real-time 24/7 monitoring</li><li>Immediate issue resolution</li><li>Payment &amp; data security covered</li><li>API &amp; integrations maintained</li></ul>'
    +'</div>';
  h+='</div>';
  h+='<h2 style="'+H2+'">Additional Upsell Opportunities</h2>';
  h+='<div class="alert"><strong>Timing:</strong> Pitch these within <strong>2 weeks</strong> of closing &mdash; while momentum is high.</div>';
  h+='<ul class="upsell">'
    +'<li><strong>CMS Integration</strong> &mdash; Client can update their own content without calling us.</li>'
    +'<li><strong>AI Chatbot / Lead Intake</strong> &mdash; Captures after-hours enquiries automatically.</li>'
    +'<li><strong>Booking System</strong> &mdash; Full appointment scheduling with calendar sync. Big value for trade businesses.</li>'
    +'<li><strong>E-commerce</strong> &mdash; Add online payments for deposits or job quotes.</li>'
    +'<li><strong>Custom Features</strong> &mdash; We can build virtually anything. Custom quote after a quick requirements chat.</li>'
    +'</ul>';
  h+='</div></body></html>';
  return h;
}
